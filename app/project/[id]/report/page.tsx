...
export default function ReportPage() {
  ...
  const healthScore = analysisRun.health_score ?? 0;
  const healthColorClass = getHealthScoreColorClass(healthScore);
    ...
    <div className="container mx-auto py-8 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Analysis Report</p>
        </div>
        <Button 
          onClick={handleExportPDF} 
          disabled={exporting}
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-8">
                <div className={`text-6xl font-bold ${healthColorClass}`}>
                  {healthScore}
                </div>
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm">
                      <span className="font-semibold">{analysisRun.passed}</span> passed
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <XCircle className="h-5 w-5 text-red-600" />
                    <span className="text-sm">
                      <span className="font-semibold">{analysisRun.failed}</span> failed
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <AlertCircle className="h-5 w-5 text-gray-600" />
                    <span className="text-sm">
                      <span className="font-semibold">{analysisRun.total_tests}</span> total
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="relative w-40 h-40">
              <svg className="w-full h-full transform -rotate-90">
                ...
                <circle
                  ...
                  className={healthScore >= 80 ? 'text-green-600' : healthScore >= 50 ? 'text-yellow-600' : 'text-red-600'}
                  strokeLinecap="round"
                />
              </svg>
              <div className="absolute inset-0 flex items-center justify-center">
                <span className={`text-3xl font-bold ${healthColorClass}`}>
                  {healthScore}
                </span>
              </div>
            </div>
        ...
        <CardContent>
          <Tabs value={selectedCategory} onValueChange={setSelectedCategory}>
            <TabsList className="grid w-full grid-cols-5">
              ...
              <TabsTrigger value="prd_compliance">
                PRD ({getCategoryCount('prd_compliance')})
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value={selectedCategory} className="mt-6 space-y-4">
              {filteredFindings.length === 0 ? (
                ...
                filteredFindings.map((finding) => (
                  <FindingCard key={finding.id} finding={finding} projectId={projectId} />
                ))
              )}
            </TabsContent>
          </Tabs>
